...
namespace rw
{
	namespace imet
	{
		...
		void ModelEngine_Yolov11_det::init(std::string enginePath, nvinfer1::ILogger& logger)
		{
			...
			// TensorRT 8.6 使用 getBindingDimensions 而不是 getTensorShape
			_input_h = _engine->getBindingDimensions(0).d[2];
			_input_w = _engine->getBindingDimensions(0).d[3];
			_detection_attribute_size = _engine->getBindingDimensions(1).d[1];
			_num_detections = _engine->getBindingDimensions(1).d[2];
		...
		{
			// TensorRT 8.6 使用 enqueueV2 API
			void* bindings[] = { _gpu_buffers[0], _gpu_buffers[1] };
			this->_context->enqueueV2(bindings, 0, nullptr);
		}
