...
namespace rw
{
	namespace imet
	{
		...
		void ModelEngine_yolov11_det_cudaAcc::init_engineRuntime(const std::string& enginePath, nvinfer1::ILogger& logger)
		{
			...
			cudaCheck(cudaStreamCreate(&_stream), "cudaStreamCreate");
		}
		...
		{
			cudaCheck(cudaMalloc(reinterpret_cast<void**>(&_deviceInputBuffer), _inputSize * sizeof(float)), "cudaMalloc _deviceInputBuffer");
			cudaCheck(cudaMalloc(reinterpret_cast<void**>(&_deviceOutputBuffer), _outputSize * sizeof(float)), "cudaMalloc _deviceOutputBuffer");
			cudaCheck(cudaMalloc(reinterpret_cast<void**>(&_deviceTransposeBuffer), _outputSize * sizeof(float)), "cudaMalloc _deviceTransposeBuffer");
			cudaCheck(cudaMalloc(reinterpret_cast<void**>(&_deviceDecodeBuffer), (1 + kMaxNumOutputBbox * kNumBoxElement) * sizeof(float)), "cudaMalloc _deviceDecodeBuffer");
		...
		{
			if (_config.classids_nms_together.empty())
			{
				return;
			}

			cudaCheck(cudaMalloc((void**)&_deviceClassIdNmsTogether, _config.classids_nms_together.size() * sizeof(size_t)), "cudaMalloc _deviceClassIdNmsTogether");
			cudaCheck(cudaMemcpy(_deviceClassIdNmsTogether, _config.classids_nms_together.data(), _config.classids_nms_together.size() * sizeof(size_t), cudaMemcpyHostToDevice), "cudaMemcpy _deviceClassIdNmsTogether");
		}

		void ModelEngine_yolov11_det_cudaAcc::destroy_cfg()
		{
			if (_deviceClassIdNmsTogether)
			{
				cudaFree(_deviceClassIdNmsTogether);
				_deviceClassIdNmsTogether = nullptr;
			}
		}

		void ModelEngine_yolov11_det_cudaAcc::warm_up()
		{
			// 关键：确保输入确定性（避免未初始化 GPU 内存导致输出不稳定/错误）
			cudaCheck(cudaMemsetAsync(_deviceInputBuffer, 0, _inputSize * sizeof(float), _stream), "cudaMemsetAsync _deviceInputBuffer");

			for (int i = 0; i < 20; i++)
			{
				this->infer();
			}
			cudaCheck(cudaStreamSynchronize(_stream), "cudaStreamSynchronize warm_up");
		}

		void ModelEngine_yolov11_det_cudaAcc::preprocess(const cv::Mat& mat)
		{
			// 对齐 CPU 版 blobFromImage(..., swapRB=true)，确保模型看到的是 RGB（如果模型导出时就是 RGB）
			// 若你的模型实际期望 BGR，可删除此转换。
			cv::Mat rgb;
			...
			cfg.dstHeight = (int)_inputHeight;
			cfg.dstWidth = (int)_inputWidth;
			cfg.pad_b = pad_b;
			cfg.pad_g = pad_g;
			cfg.pad_r = pad_r;

			_letterBoxInfo = ImgPreprocess::LetterBox(rgb, cfg, _stream);
			cudaCheck(cudaGetLastError(), "ImgPreprocess::LetterBox kernel launch");
		}
		...
		void ModelEngine_yolov11_det_cudaAcc::infer()
		{
			...
			Utility::nms_det(
				...
				(int)_config.classids_nms_together.size(),
				_stream);

			cudaCheck(cudaMemcpyAsync(_hostOutputBuffer, _deviceDecodeBuffer, (1 + kMaxNumOutputBbox * kNumBoxElement) * sizeof(float), cudaMemcpyDeviceToHost, _stream), "cudaMemcpyAsync _hostOutputBuffer");
			cudaCheck(cudaStreamSynchronize(_stream), "cudaStreamSynchronize infer");
		}
